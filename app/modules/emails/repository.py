...
def count_emails_by_category(user_id: str, category: str) -> int:
    db = get_supabase()
    email_ids_res = db.table("emails").select("id").eq("user_id", user_id).execute()
    all_ids = [e["id"] for e in email_ids_res.data]
    if not all_ids:
        return 0
    res = (
        db.table("email_categories")
